...
import { Button, Image, Input, ScrollView, Switch, Text, Textarea, View } from '@tarojs/components';
...
import './addGeneralEvent.scss';

const AddGeneralEvent = () => {
  // 获取路由参数
  ...
  useEffect(() => {
    Taro.setNavigationBarTitle({
      title: isEditMode ? '编辑通用事件' : '添加通用事件'
    });
  }, [isEditMode]);
    ...
    try {
      ...
      if (response) {
        ...
        setFormData({
          ...
          event_type: response.eventType || 'common',
  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      try {
        ...
        console.error('Failed to save general event:', error);
      }
    }
  };
  ...
  return (
    <View style={{ width: '100%', height: '100vh', backgroundColor: '#f5f5f5', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <ScrollView 
        className='add-general-event-container' 
        scrollY 
        enableBackToTop
      ...
      >
        ...
        <View className='general-event-form' style={{ width: '100%', boxSizing: 'border-box' }}>
        {/* 基本信息 */}
        <View className='form-section'>
          ...
          <View className='form-item'>
            <Text className='form-label'>事件日期 *</Text>
            <DateTimePicker
          ...
          <View className='form-item'>
            <Text className='form-label'>事件内容 *</Text>
            <View className={`content-container ${errors.content ? 'error' : ''}`}>
              <Textarea
              ...
              <View className='word-count'>
                <Text>{formData.content ? formData.content.length : 0}/1000</Text>
              </View>
            </View>
            {errors.content && <Text className='error-text'>{errors.content}</Text>}
          </View>
        </View>
