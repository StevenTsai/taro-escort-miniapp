...
import { checkConfig, commonRequest, getCloudInstance, randomSelectInitquestion, sleep } from "./tools";
Component({
  properties: {
    chatMode: {
      type: String,
  ...
  methods: {
    ...
    _throttledSetData: function (data) {
      this._pendingData = Object.assign(this._pendingData || {}, data);
      if (!this._dataFlushTimer) {
    ...
    _flushPendingData: function () {
      if (this._dataFlushTimer) {
        clearTimeout(this._dataFlushTimer);
        this._dataFlushTimer = null;
      }
      if (this._pendingData && Object.keys(this._pendingData).length > 0) {
        const pending = this._pendingData;
      ...
      if (chatMode === "model") {
        ...
        for await (let event of res.eventStream) {
          ...
          try {
            ...
            this._throttledSetData({ chatRecords: newValue, chatStatus });
          } catch (e) {
            // console.log(e, event)
            break;
          }
